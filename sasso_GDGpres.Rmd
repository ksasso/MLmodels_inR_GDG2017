...
- Set $\hat{f}_{1}(x)=0$ and $r_{i},_{1} = y_{i}$ for all _i_ in the training set at stage one
- For _k_ = 1,2,..., _K_, repeat:
    + Fit a tree $\hat{t}^{k}$ with _d_ splits to the training data
    + Update $\hat{f}$ by adding in a shrunken version of the new tree 
\[
\hat{f}_{k+1}(x_{i}) \leftarrow \hat{f}_{k}(x_{i}) + \lambda\hat{t}_{k}(x_{i}) 
\]
    + Update the residuals
\[
r_{i},_{k+1}\leftarrow r_{i},_{k}-\lambda\hat{f}_{k}(x_{i})
\]
    + Output the boosted model
\[
\hat{f}(x) = \sum_{k=1}^{K}\lambda\hat{t}_{k}(x)
\]
</div>
